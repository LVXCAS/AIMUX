...
const DESKTOP_THREAD_OPENED_MESSAGE: &str = "Opened this session in AIMUX Desktop.";
...
fn desktop_thread_open_error_message(err: &str) -> String {
    format!(
        "Failed to open this session in AIMUX Desktop: {err}. Install or launch AIMUX Desktop and try again."
    )
}
...
fn open_desktop_thread_url(url: &str) -> Result<(), String> {
    ...
    let output = std::process::Command::new("powershell.exe")
        ...
        .arg(&script)
        .output()
        .map_err(|err| format!("failed to launch AIMUX Desktop through PowerShell: {err}"))?;
    ...
    if stderr.is_empty() {
        Err(format!(
            "failed to launch AIMUX Desktop through PowerShell with {}",
            output.status
        ))
    } else {
        Err(stderr)
...
if ([string]::IsNullOrWhiteSpace($installLocation)) {{
    Write-Error 'AIMUX Desktop package is not installed'
    exit 1
}}
...
if (-not (Test-Path $exe)) {{
    Write-Error "AIMUX Desktop executable not found at $exe"
    exit 1
}}
if (-not (Test-Path $app)) {{
    Write-Error "AIMUX Desktop app bundle not found at $app"
    exit 1
}}
...
fn open_desktop_thread_url(_url: &str) -> Result<(), String> {
    Err("AIMUX Desktop is only available on macOS and Windows".to_string())
}
